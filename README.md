...
An LSTCN model [1] can be defined as a recurrent neural network composed of sequentially-ordered Short-term Cognitive Network (STCN) blocks [2]. Each STCN block is a two-layer neural network that uses shallow learning to fit the local model to a specific time patch, which is then transfered to the following block. Time patches are temporal pieces of data resulting from partitioning the time series. 
